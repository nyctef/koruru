...
typedef struct AudioFileState {
	 ...
	 */
	AudioFileState(const char* filename);
 ...
 */
double inline get_master_clock() { return av_gettime() / 1000000.0; }

/** \brief Where most of the action happens
 * 
 * Runs in a separate thread, initialises stuff then runs the main loop.
 * 
 * Data: file -> PacketQueue -> (decoding) -> AudioBuffer -> SDL's buffer -> hardware
 * \param arg An AudioFileState pointer
 * \return 0
 */
